...
function GameTable() {
  ...
  const gridLine = "rgb(29 95 168 / 0.32)";
  const bgLayers = [
  ...
  return (
    <div className="flex flex-col lg:h-[calc(100dvh-3.5rem)] lg:flex-row">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex flex-wrap items-center gap-3 border-b border-module-soft bg-white px-4 py-2">
          ...
          {isGM && <span className="rounded bg-module-soft px-2 py-0.5 text-xs font-bold text-module-deep">Você é o mestre</span>}
          ...
          <button className="btn btn-ghost border border-module-soft" onClick={copyLink}>
        ...
        <div className="h-[65vh] overflow-auto bg-[#e8eef5] p-6 lg:h-auto lg:flex-1">
          <div
            ref={boardRef}
            className="relative mx-auto select-none shadow-[0_0_0_1px_rgb(29_95_168/0.5)]"
            style={{
            ...
            {tokens.map((t) => {
              ...
              return (
                <button
                  ...
                  className={`absolute flex items-center justify-center rounded-full font-display font-bold text-white ${
                  ...
                  style={{
                    ...
                    fontSize: cell * 0.32,
                    background: t.color,
                    border: `${Math.max(2, cell * 0.05)}px solid ${isSel ? "#172233" : "#fff"}`,
                    boxShadow: isSel ? "0 0 0 3px rgb(29 95 168 / .35)" : "0 1px 3px rgb(23 34 51 / .35)",
      ...
      {/* ---------- Painel lateral ---------- */}
      <aside className="w-full shrink-0 space-y-6 overflow-y-auto border-l border-module-soft bg-paper p-4 lg:w-[340px]">
        <section>
          <h2 className="mb-2 font-display text-lg font-bold">Na mesa ({online.length})</h2>
          <ul className="flex flex-wrap gap-1.5">
            {online.map((p) => (
              <li key={p.user_id} className="rounded-full bg-white px-3 py-1 text-sm shadow-[0_0_0_1px_var(--color-module-soft)]">
          ...
          <section className="panel p-3">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {selectedToken.character_id && selectedToken.owner_id === user.id && (
                <Link href={`/fichas/${selectedToken.character_id}`} target="_blank" className="btn btn-ghost border border-module-soft">
                  Abrir ficha
                </Link>
              )}
            ...
            {rolls.map((r) => (
              <li
                ...
                className={`rounded-md border bg-white px-3 py-2 ${
                  r.crit === "critico" ? "border-moss" : r.crit === "falha" ? "border-blood" : "border-module-soft"
                }`}
              >
                <div className="flex items-baseline justify-between gap-2">
                  ...
                  <span
                    className={`font-display text-2xl font-extrabold ${
                      r.crit === "critico" ? "text-moss" : r.crit === "falha" ? "text-blood" : "text-module-deep"
                    }`}
                  >
                    {r.total}
                  </span>
      ...
      <div className="flex gap-1.5" role="radiogroup" aria-label="Cor do token">
        {TOKEN_COLORS.map((c) => (
          <button
            ...
            style={{ background: c, outline: color === c ? "2px solid #172233" : "none", outlineOffset: 2 }}
          />
        ))}
      </div>
...
function RoomSettings({ room, onSave }: { room: Room; onSave: (p: Partial<Room>) => Promise<void> }) {
  ...
  return (
    <form
      className="space-y-2 border-t border-module-soft pt-5"
      onSubmit={async (e) => {
        e.preventDefault();
        await onSave({
